  ...
  return (
    <div className="hours">
      <Scrollbars
        ...
        className="hours__content-wrapper">
        <ButtonGroup className="hours__container d-grid">
          {hoursMassive.map((index) => {
            ...
            const hourHandler = () => {
              if (!chosenHourses.current.includes(hourValue)) {
                ...
                chosenHourses.current = chosenHourses.current.filter(
                  (e) => e !== hourValue
                ) // delete element from sending massiv
              }
...
function Hour({
  ...
  isChosen,
  onClick
}: HourProps): JSX.Element {
  return (
    <Button
      onClick={onClick}
      variant={
        isOccupied ? 'outline-danger' : isChosen ? 'success' : 'outline-primary'
      }
      id={`hour${mainContent}`}
      disabled={isOccupied}
      value={mainContent}
