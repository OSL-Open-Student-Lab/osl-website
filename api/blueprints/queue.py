...
def queries():
    try:
        if request.method == 'POST':
            ...
            booking_timedelta = str_to_time(queue.to_date) -\
                str_to_time(queue.from_date)

            if booking_timedelta.days > 0 or\
               booking_timedelta.seconds >= 28800:
                return jsonify('booking time is too long'), 400

            try:
                check_facility = Facilities.query.\
                    filter_by(id=queue.facility_id).all()
...
def get_bookings_by_params(facility_id, date):
    ...
    try:
        bookings_by_id = FacilityBooking.query.\
            filter_by(facility_id=facility_id).all()
        avaliable_bokings = []
